...
namespace NugieApp {
  RayTraceImage::RayTraceImage(NugieVulkan::Device* device, uint32_t width, uint32_t height, uint32_t imageCount) {
  ...
  void RayTraceImage::createRayTraceImages(NugieVulkan::Device* device, uint32_t width, uint32_t height, uint32_t imageCount) {
   	this->rayTraceImages.clear();

		for (uint32_t i = 0; i < imageCount; i++) {
			auto rayTraceImage = NugieVulkan::Image(
				device, width, height, 
	...
	void RayTraceImage::prepareFrame(NugieVulkan::CommandBuffer* commandBuffer, uint32_t frameIndex) {
		if (this->rayTraceImages[frameIndex].getLayout() == VK_IMAGE_LAYOUT_UNDEFINED) {
			this->rayTraceImages[frameIndex].transitionImageLayout(VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL,
				...
				0, VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT, VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED,
				commandBuffer);
		} else {
	...
	void RayTraceImage::transferFrame(NugieVulkan::CommandBuffer* commandBuffer, uint32_t frameIndex) {
		this->rayTraceImages[frameIndex].transitionImageLayout(VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL,
			VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 
			VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_SHADER_READ_BIT, VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED,
			commandBuffer);
	}

	void RayTraceImage::finishFrame(NugieVulkan::CommandBuffer* commandBuffer, uint32_t frameIndex) {
		this->rayTraceImages[frameIndex].transitionImageLayout(VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL,
