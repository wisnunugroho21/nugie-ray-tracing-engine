...
namespace NugieApp {
	class AccumulateImage {
		public:
			AccumulateImage(NugieVulkan::Device* device, uint32_t width, uint32_t height, uint32_t imageCount);
		...
		private:
			...
			void createAccumulateImages(NugieVulkan::Device* device, uint32_t width, uint32_t height, uint32_t imageCount);
	};
