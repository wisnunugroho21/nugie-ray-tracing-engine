...
namespace nugiEngine {
  ...
  CameraTransformation EngineMouseController::rotateInPlaceXZ(GLFWwindow* window, float dt, CameraTransformation cameraTransformation, bool* isPressed) {
    if (glfwGetMouseButton(window, this->keymaps.rightButton) == GLFW_PRESS) {
      ...
      float phi = glm::radians((curDragged_y - this->lastDragged_y) * dt * this->lookSpeed);
      float theta = glm::radians((curDragged_x - this->lastDragged_x) * dt * this->lookSpeed);
      ...
      cameraTransformation.direction.y = glm::cos(theta) * cameraTransformation.direction.y - glm::sin(phi) * cameraTransformation.direction.z;
      cameraTransformation.direction.z = glm::sin(theta) * cameraTransformation.direction.y + glm::cos(phi) * cameraTransformation.direction.z;
